...
public class Main {
	public static void main(String args[]) {
		...
		try {
			...
			try {
				...
				db = DriverManager.getConnection("jdbc:postgresql://localhost/music", "chris", "test");
			...
			JdbcExercise service = new JdbcExercise(db);
			...
			if (cmd.equals("ShowTrack") && args.length == 2) {
				try {
					...
					try {
						...
						System.err.println("track does not exist.");
						System.err.println(e.getMessage());
						e.printStackTrace();
					}
				...
				try {
					...
					System.out.println();
					System.out.println(String.format("%-20s | %-10s | %-10s", "Name", "Released", "Price"));
					System.out.println("==============================================");
				...
				try {
					...
					System.out.println(String.format("%-10s | %-10s | %-17s | %-10s", user.getName(),
							user.getUsername(), user.getRegistrationDate().toLocalDate(), user.isActive()));
				} catch (IllegalArgumentException e) {
					System.err.println(e.getMessage());
					e.printStackTrace();
				}
				...
				try {
					...
					for (Purchase purchase : purchases) {
						System.out.print(String.format("%-10s | %-6s | ", 
								purchase.getPurchaseDate().toLocalDate(), purchase.getPrice()));
	...
	public static void usage() {
		...
		System.out.println("\tjava uk.ac.bham.cs.jdbc.music.Main ShowUser <username>");
		System.out.println("\tjava uk.ac.bham.cs.jdbc.music.Main AddToBasket <username>");
		System.out.println("\tjava uk.ac.bham.cs.jdbc.music.Main GetPurchases <username>");
		System.out.println("\tjava uk.ac.bham.cs.jdbc.music.Main Purchase <username>");
	}
}
