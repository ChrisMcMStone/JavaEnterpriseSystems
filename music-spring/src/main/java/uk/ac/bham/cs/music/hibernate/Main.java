	...
	public static void main(String[] args) {
		try {
			...
			try (AbstractApplicationContext context = new ClassPathXmlApplicationContext("music-spring.xml")) {
				...
				if (args.length < 1) {
					Main.usage(); // no? then tell everyone how it should be
									// done.
					return;
				}
				...
				HibernateExercise service = (HibernateExercise) context.getBean("musicService");
				...
				if (cmd.equals("ShowTrack") && args.length == 2) {
					try {
						...
						try {
							...
							PeriodFormatter length = new PeriodFormatterBuilder().printZeroAlways()
				...
				} else if (cmd.equals("InsertArtist") && args.length == 3) {
					...
					try {
						...
						System.out.println(String.format("%-20s | %-20s | %-10s | %-10s", "Name", "Username",
								"Registration Date", "Active?"));
						System.out.println("=========================================================================");
						DateTimeFormatter df = DateTimeFormat.forPattern("dd-MM-yyyy");
						System.out.println(String.format("%-20s | %-20s | %10s | %10s", user.getName(),
								user.getUsername(), df.print(user.getRegistrationTime()), user.isActive()));
					} catch (IllegalArgumentException e) {
						System.err.println(e.getMessage());
					}
					...
					try {
						String username = args[1];
						int trackID = Integer.parseInt(args[2]);
						service.addToBasket(username, trackID);
						System.out.println("Track added to user's basked successfully.");
					} catch (IllegalArgumentException e) {
						System.err.println(e.getMessage());
					}
					...
					try {
						String username = args[1];
						service.purchase(username);
						System.out.println("Tracks purchased successfully.");
					} catch (IllegalArgumentException e) {
						System.err.println(e.getMessage());
					}
				} else if (cmd.equals("AddReview") && args.length == 5) {
					try {
						service.addReview(args[1], args[2], args[3], args[4]);
						System.out.println("Review successfully added");
					} catch (IllegalArgumentException e) {
						System.err.println(e.getMessage());
					}
				} else if (cmd.equals("populate") && args.length == 1) {
					service.populate();
				} 
			}
